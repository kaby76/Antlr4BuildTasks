...
namespace SharpCompress.Writers.Zip
{
    public class ZipWriter : AbstractWriter
    {
        ...
        private PpmdProperties PpmdProperties
        {
            get
            {
                if (ppmdProps == null)  ppmdProps = new PpmdProperties();
                return ppmdProps;
            }
        }
        ...
        public override void Write(string entryPath, Stream source, Nullable<DateTime> modificationTime)
        {
            Write(entryPath, source, new ZipWriterEntryOptions()
            {
                ModificationDateTime = modificationTime.Value
            });
        }
        ...
        public Stream WriteToStream(string entryPath, ZipWriterEntryOptions options)
        {
            ...
            if (options.ModificationDateTime == null) options.ModificationDateTime = DateTime.Now;
            if (options.EntryComment == null) options.EntryComment = string.Empty;
