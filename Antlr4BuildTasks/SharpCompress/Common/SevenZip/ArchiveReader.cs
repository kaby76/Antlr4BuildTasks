    ...
    internal class ArchiveReader
    {
                                        ...
                                        out List<long> unpackSizes, out List<uint?> digests)
        {
            ...
            try
            {
                ...
                for (; ; )
                {
                    ...
                    if (type is BlockType.Crc || type is BlockType.Size)
                    {
                        break;
                    }
                    if (type == BlockType.End)
                ...
                for (; ; )
                {
                    ...
                    switch (type)
                    {
                        ...
                        case BlockType.CTime:
                            ...
                            Log.Write("CTime:");
#endif
                            ReadDateTimeVector(dataVector, numFiles,
                                delegate (int i, Nullable<DateTime> time)
                                                                     {
        ...
        public void Extract(ArchiveDatabase db, int[] indices)
        {
            ...
            foreach (CExtractFolderInfo efi in extractFolderInfoVector)
            {
                ...
                if (buffer == null) buffer = new byte[4 << 10];
                for (; ; )
                {
